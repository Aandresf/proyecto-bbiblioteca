...
CREATE VIEW "LIBROS DISPONIBLES" AS
	SELECT CODIGO AS "CODIGO",
		...
		(SELECT ( SELECT DETALLES FROM AUTORES AS AUT WHERE AUT.ID = LIB.AUTOR_LIBRO) 
			FROM LIBROS AS LIB WHERE LIB.ID = INV.LIBRO ) AS "AUTOR",

		(SELECT ( SELECT DETALLES FROM CATEGORIA_LIBROS AS CAT WHERE CAT.ID = LIB.CATEGORIA) 
			FROM LIBROS AS LIB WHERE LIB.ID = INV.LIBRO ) AS "CATEGORIA",
...
CREATE VIEW "DETALLES SECTORES" AS
	SELECT ID AS "ID SECTOR", SECTOR AS "NOMBRE",
		(SELECT DESCRIPCION FROM BIBLIOTECAS AS BIB WHERE BIB.ID = SEC.BIBLIOTECA) AS "BIBLIOTECA",
		(SELECT COUNT(*) FROM INVENTARIO AS INV WHERE INV.UBICACION = SEC.ID) AS "LIBROS"
	FROM SECTORIZACION AS SEC ORDER BY "NOMBRE" ASC;
